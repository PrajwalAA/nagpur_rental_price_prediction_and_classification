...
def load_resources():
    ...
    try:
        # Load Model files using the new names
        rf_model = joblib.load('mm2.pkl')
        scaler = joblib.load('ss2.pkl')
        features = joblib.load('ff2.pkl')
        st.success("Model (mm2.pkl) and its resources loaded successfully.")
        return rf_model, scaler, features
    except FileNotFoundError as e:
        st.error(f"Error: A required file was not found. Please ensure 'mm2.pkl', 'ss2.pkl', and 'ff2.pkl' are in the same directory.")
        st.info(f"Details: {e}")
...
if rf_model is not None and scaler is not None and features is not None:
    ...
    st.warning("Cannot run prediction. Please ensure all model files ('mm2.pkl', 'ss2.pkl', and 'ff2.pkl') are available in the same directory.")
